...
//----------------------------------------------------------------------------//
	//Initialising The radio module
		...
		printf("[Init] BCM2835 GPIO : OK \t  CS = GPIO %d \t IRQ = GPIO %d \t RST = GPIO %d\n", RF_CS_PIN,RF_IRQ_PIN,RF_RST_PIN);
			...
			if (bcm2835_gpio_eds(RF_IRQ_PIN)){
				...
				if (rf95.available()){
					...
					if (manager_routing.recvfromAck(buf, &len, &from, &to)){
						...
						printf("Message : %s\n\n",buf);
						struct message msg = build_udp_packet(0,buf);
						memcpy(up.payload,msg.complete_msg,msg.size);
						up.len = msg.size;
